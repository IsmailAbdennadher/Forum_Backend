...
public interface LikePostRepository extends JpaRepository<LikePost, Long> {
    ...
    @Query(value = "SELECT count(*) + (select count(distinct like_post.post_id) as test from forum.like_post where like_post.post_id = ?1) FROM forum.like_post as lp right join forum.post as p on lp.post_id=p.id \n" +
